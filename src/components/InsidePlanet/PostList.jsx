...
const PostList = ({title,time,id,planetOwner}) => {
    ...
    const onClickDeleteBtn = async() => {
        ...
        await axios.delete(`http://52.78.18.110:8000/deleteletter`,config)
        .then((res)=>{
            console.log(res);
    ...
    return (
        <div className="PostList" ref={PostListRef} onClick={onClickPostList}>
            ...
            <div className="PostListTime" ref={PostListTimeRef}>{time.slice(0, 10)}</div>

            {isListClicked? (
                null
            ) : (
            <div className="PostDelteBtn" onClick={onClickDeleteBtn}>DELETE</div>
            )}
        </div>
    )
}
